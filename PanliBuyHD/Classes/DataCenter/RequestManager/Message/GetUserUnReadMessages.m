...
@implementation GetUserUnReadMessages
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_GETUSERUNREADMESSAGES];
...
- (void)receive:(ASIHTTPRequest *)request
{
    ...
    if (errorInfo.code == API_SUCCESS)
    {
        ...
        UserUnReadMessages *mUserUnReadMessages = [[UserUnReadMessages alloc] init];
        mUserUnReadMessages.userId = [PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"UserId"];
        mUserUnReadMessages.customerMsgCount = [[PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"UnreadCustomerMessageCount"] intValue];
        mUserUnReadMessages.sysmessMsgCount = [[PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"UnreadSystemMessages"] intValue];
        mUserUnReadMessages.productObjIds = [PanliHelper getValue:Q_TYPE_ARRAY inJsonDictionary:dic propertyName:@"UnreadProductCustomerMessageObjIds"];
        mUserUnReadMessages.shipObjIds = [PanliHelper getValue:Q_TYPE_ARRAY inJsonDictionary:dic propertyName:@"UnreadShipCustomerMessageObjIds"];
        mUserUnReadMessages.parcelObjIds = [PanliHelper getValue:Q_TYPE_ARRAY inJsonDictionary:dic propertyName:@"UnreadParcelCustomerMessageObjIds"];
        ((DataRepeater*)self.repeater).isResponseSuccess = YES;
        //设置响应数据
        ((DataRepeater*)self.repeater).responseValue = mUserUnReadMessages;
        [mUserUnReadMessages release];
    }
    else
    {

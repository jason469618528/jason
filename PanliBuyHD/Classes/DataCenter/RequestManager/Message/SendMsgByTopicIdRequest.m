...
@implementation SendMsgByTopicIdRequest
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_SENDMESSAGEBYTOPICID];
...
- (void)receive:(ASIHTTPRequest *)request
{
    ...
    {
        //接口响应成功
        NSString *str_Date = [jsonValue objectForKey:@"Date"];
        ((DataRepeater*)self.repeater).isResponseSuccess = YES;
        //设置响应数据
        ((DataRepeater*)self.repeater).responseValue = str_Date;
    }
    else
    {
...
- (ErrorInfo *)checkResponseError:(NSDictionary*)jsonValue
{
    ...
    if (errorInfo.code == SERVER_SUCCESS)
    {
        ...
        {
            errorInfo.code = 2;
            errorInfo.message = LocalizedString(@"SendMsgByTopicIdRequest_ErrorInfo_code2",@"短信主题ID无效");
        } else if ([code intValue] == 3)
        {
            errorInfo.code = 3;
            errorInfo.message = LocalizedString(@"SendMsgByTopicIdRequest_ErrorInfo_code3",@"短信内容为空");
        }  else if ([code intValue] == 4)
        {
            errorInfo.code = 4;
            errorInfo.message = LocalizedString(@"SendMsgByTopicIdRequest_ErrorInfo_code4",@"对象ID无效");
        }
        else if ([code intValue] == 5)
        {
            errorInfo.code = 5;
            errorInfo.message = LocalizedString(@"SendMsgByTopicIdRequest_ErrorInfo_code5",@"发送失败");
        }
        else
        {

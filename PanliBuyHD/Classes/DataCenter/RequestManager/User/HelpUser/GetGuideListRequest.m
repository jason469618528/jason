...
@implementation GetGuideListRequest
- (id)init
{
    self = [super init];
    if (self)
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_GETGUIDE];
...
- (void)receive:(ASIHTTPRequest *)request
{
    ...
    if (errorInfo.code == API_SUCCESS)
    {
        ...
        for(NSDictionary *guideDic in resultArray)
        {
            Guide *mGuide = [[Guide alloc] initWithDictionary:guideDic];
            [guideInfo addObject:mGuide];
            [mGuide release];
        }
        
        ((DataRepeater*)self.repeater).isResponseSuccess = YES;
        ((DataRepeater*)self.repeater).responseValue = guideInfo.count > 0 ? guideInfo : nil;
        [guideInfo release];
    }
    else
    {

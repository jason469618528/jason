...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_SHOPCART_PAY];
...
- (ErrorInfo *)checkResponseError:(NSDictionary*)jsonValue
{
    ...
    if (errorInfo.code == SERVER_SUCCESS)
    {
        ...
        {
            errorInfo.code = 5;
            errorInfo.message = LocalizedString(@"Common_ErrorInfo_code5",@"无效用户");
        }
        else if ([code intValue] == 6)
        {
            errorInfo.code = 6;
            errorInfo.message = LocalizedString(@"Common_ErrorInfo_code6",@"余额不足");
        }

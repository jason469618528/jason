//
//  SetCartProductRemarkRequest.m
//  PanliApp
//
//  Created by jason on 13-5-9.
//  Copyright (c) 2013年 Panli. All rights reserved.
//

#import "SetCartProductRemarkRequest.h"

@implementation SetCartProductRemarkRequest
- (id)init
{
    self = [super init];
    if (self)
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_SHOPCART_REMARKS];
...
- (ErrorInfo *)checkResponseError:(NSDictionary*)jsonValue
{
    ...
    if (errorInfo.code == SERVER_SUCCESS)
    {
        ...
        {
            errorInfo.code = 3;
            errorInfo.message = LocalizedString(@"SetCartProductRemarkRequest_ErrorInfo_code3",@"商品ID无效");
        }
        else if ([code intValue] == 4)
        {
            errorInfo.code = 4;
            errorInfo.message = LocalizedString(@"SetCartProductRemarkRequest_ErrorInfo_code4",@"该商品不能修改");
        }

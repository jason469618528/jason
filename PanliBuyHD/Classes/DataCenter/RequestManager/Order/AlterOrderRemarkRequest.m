...
#import "AlterOrderRemarkRequest.h"

@implementation AlterOrderRemarkRequest
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_PRODUCTREMARK];
...
- (ErrorInfo *)checkResponseError:(NSDictionary*)jsonValue
{
    ...
    if (errorInfo.code == SERVER_SUCCESS)
    {
        ...
        {
            errorInfo.code = 2;
            errorInfo.message = LocalizedString(@"AlterOrderRemarkRequest_ErrorInfo_code2",@"保存失败");
        }
        else if ([code intValue] == 3)
        {
            errorInfo.code = 3;
            errorInfo.message = LocalizedString(@"AlterOrderRemarkRequest_ErrorInfo_code3",@"商品ID无效");
        }
        else if ([code intValue] == 4)
        {
            errorInfo.code = 4;
            errorInfo.message = LocalizedString(@"AlterOrderRemarkRequest_ErrorInfo_code4",@"该商品不能修改备注");
        }

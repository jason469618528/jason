...
@implementation UserShareProductsRequest
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:@"Share/UserShareProducts.json"];
...
- (void)receive:(ASIHTTPRequest *)request
{
    ...
    if (errorInfo.code == API_SUCCESS)
    {
        ...
        NSArray *arr_result = [dic_result objectForKey:@"list"];
        
        if(arr_result != nil)
        {
            NSMutableArray *mArr_productList = [[NSMutableArray alloc] init];
            for(NSDictionary *dic in arr_result)
            {
                ShareProduct *mShareProduct = [[ShareProduct alloc] initWithDictionary:dic];
                [mArr_productList addObject:mShareProduct];
                [mShareProduct release];
            }
            mPageData.list = mArr_productList;
            [mArr_productList release];
        }
        mPageData.hasNext = [[dic_result objectForKey:@"hasNext"] boolValue];

...
@implementation ShareTopicsRequest
- (id)init
{
    self = [super init];
    if (self)
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:@"Share/ShareTopics.json"];
...
- (void)receive:(ASIHTTPRequest *)request
{
    ...
    if (errorInfo.code == API_SUCCESS)
    {
        ...
        //接口响应成功
        NSArray *resultArray = [PanliHelper getValue:Q_TYPE_ARRAY inJsonDictionary:jsonValue propertyName:RP_PARAM_RESULT];
        NSMutableArray *marr_ShareInfo = [[NSMutableArray alloc] init];
        
        for(NSDictionary *advertDic in resultArray)
        {
            ShareBuyTopic *mShareBuyTopic = [[ShareBuyTopic alloc] initWithDictionary:advertDic];
            [marr_ShareInfo addObject:mShareBuyTopic];
            [mShareBuyTopic release];
        }
        //接口响应成功
        ((DataRepeater*)self.repeater).isResponseSuccess = YES;
        //设置响应数据
        ((DataRepeater*)self.repeater).responseValue = marr_ShareInfo;

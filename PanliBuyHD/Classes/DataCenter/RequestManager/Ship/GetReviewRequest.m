...
#import "ShipReview.h"
@implementation GetReviewRequest
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_SHIPREVIEW];
...
- (void)receive:(ASIHTTPRequest *)request
{
    ...
    if (errorInfo.code == API_SUCCESS)
    {
        ...
        ShipReview *m_Review = [[ShipReview alloc]init];
        m_Review.userId = [PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"UserId"];
        m_Review.generalRate = [PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"GeneralRate"];
        m_Review.receiveRate = [PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"ReceiveRate"];
        m_Review.deliveryRate = [PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"DeliveryRate"];
        m_Review.customerRate = [PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"CustomerRate"];
        m_Review.content = [PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"Content"];
        m_Review.shipID = [PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"ShipId"];
        
        
        ((DataRepeater*)self.repeater).isResponseSuccess = YES;
        //设置响应数据
        ((DataRepeater*)self.repeater).responseValue = m_Review;
        [m_Review release];
    }
    else
    {
...
- (ErrorInfo *)checkResponseError:(NSDictionary*)jsonValue
{
    ...
    if (errorInfo.code == SERVER_SUCCESS)
    {
        ...
        {
            errorInfo.code = 3;
            errorInfo.message = LocalizedString(@"GetReviewRequest_ErrorInfo_code3",@"运单编号无效");
        }

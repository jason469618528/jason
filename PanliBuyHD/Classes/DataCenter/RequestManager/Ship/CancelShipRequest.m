...
@implementation CancelShipRequest
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_CANCELSHIPORDER];
...
- (ErrorInfo *)checkResponseError:(NSDictionary*)jsonValue
{
    ...
    if (errorInfo.code == SERVER_SUCCESS)
    {
        ...
        {
            errorInfo.code = 3;
            errorInfo.message = LocalizedString(@"CancelShipRequest_ErrorInfo_code3",@"运单编号无效");
        }
        else if ([code intValue] == 12)
        {
            errorInfo.code = 12;
            errorInfo.message = LocalizedString(@"CancelShipRequest_ErrorInfo_code12",@"撤销运单失败");
        }

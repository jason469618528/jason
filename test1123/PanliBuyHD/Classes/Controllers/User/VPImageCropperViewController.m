...
{
    [super viewDidLoad];
    self.view.backgroundColor = PL_COLOR_WHITE;
    [self initView];
    [self initControlBtn];
}
...
- (void)initView {
    self.showImgView = [[[UIImageView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 320, UI_SCREEN_HEIGHT)] autorelease];
...
//    self.overlayView.alpha = .5f;
    ...
    UIImageView *img = [[UIImageView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 320, IS_568H ? 518 : 430)];
    img.image = IS_568H ? [PanliHelper getImageFileByName:@"bg_uploadShadow_h568@2x.png"] : [PanliHelper getImageFileByName:@"bg_uploadShadow@2x.png"];
    [self.view addSubview:img];
    [img release];
    
    
    self.ratioView = [[[UIView alloc] initWithFrame:self.cropFrame] autorelease];
...
- (void)initControlBtn
{
    UIView *toolBarView = [[UIView alloc] initWithFrame:CGRectMake(0.0f, self.view.frame.size.height - 50, 320, 50.0f)];
    toolBarView.backgroundColor = [PanliHelper colorWithHexString:@"#efefef"];
    [self.view insertSubview:toolBarView aboveSubview:self.overlayView];
    [toolBarView release];
    
    UIButton *btn_cancel = [UIButton buttonWithType:UIButtonTypeCustom];
    btn_cancel.frame = CGRectMake(20.0f, 12.0f, 50.0f, 26.0f);
    btn_cancel.tag = 1001;
    [btn_cancel setImage:[UIImage imageNamed:@"btn_upload_cancel"] forState:UIControlStateNormal];
    [btn_cancel setImage:[UIImage imageNamed:@"btn_upload_cancel_on"] forState:UIControlStateHighlighted];
    [btn_cancel addTarget:self action:@selector(cancel:) forControlEvents:UIControlEventTouchUpInside];
    [toolBarView addSubview:btn_cancel];
    
    UIButton *btn_submit = [UIButton buttonWithType:UIButtonTypeCustom];
    btn_submit.frame = CGRectMake(320 - 50 - 20, 12.0f, 50.0f, 26.0f);
    btn_submit.tag = 1002;
    [btn_submit setImage:[UIImage imageNamed:@"btn_upload_submit"] forState:UIControlStateNormal];
    [btn_submit setImage:[UIImage imageNamed:@"btn_upload_submit_on"] forState:UIControlStateHighlighted];
    [btn_submit addTarget:self action:@selector(confirm:) forControlEvents:UIControlEventTouchUpInside];
    [toolBarView addSubview:btn_submit];
}

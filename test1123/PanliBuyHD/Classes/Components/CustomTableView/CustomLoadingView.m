    ...
    if (self)
    {
        ...
        switch (_style)
        {
            case head_none:
            {
                ...
                _tipLabel.textColor = [PanliHelper colorWithHexString:@"#89afe9"];
                _tipLabel.shadowColor = [UIColor colorWithWhite:0.9f alpha:1.0f];
                _tipLabel.shadowOffset = CGSizeMake(0.0f, 1.0f);
                _tipLabel.backgroundColor = PL_COLOR_CLEAR;
                _tipLabel.textAlignment = UITextAlignmentCenter;
                _tipLabel.text = LocalizedString(@"CustomLoadingView_tipLabel1",@"下拉刷新...");
            ...
            case foot_none:
            {
                ...
                _tipLabel.backgroundColor = PL_COLOR_CLEAR;
                _tipLabel.textAlignment = UITextAlignmentCenter;
                _tipLabel.text = LocalizedString(@"CustomLoadingView_tipLabel2",@"上拉载入更多...");
                [self addSubview:_tipLabel];
            }
                break;
            case head_refreshTime:
            {
                ...
                else
                {
                    ...
                    [dateFormatter setDateFormat:LocalizedString(@"CustomLoadingView_dateFormatter",@"最近刷新：yyyy-MM-dd HH:mm")];
...
- (id)initWithFrame:(CGRect)frame andType:(LoadingViewStyle)style andKey:(NSString *)key andFont:(UIFont *)font
{
    ...
    _tipLabel.font = font;
    _tipLabel.shadowColor = PL_COLOR_CLEAR;
    _tipLabel.textColor = [PanliHelper colorWithHexString:@"#89afe9"];
...
- (void)changeState:(LoadingState)state
{
    switch (state) {
		case Pulling:
            ...
            if (_style != foot_none) {
                _tipLabel.text = LocalizedString(@"CustomLoadingView_tipLabel3",@"松开刷新...");
            }else{
                _tipLabel.text = LocalizedString(@"CustomLoadingView_tipLabel4",@"松开载入更多...");
            }
		...
		case Loading:
            ...
            if (_style != foot_none) {
                _tipLabel.text = LocalizedString(@"CustomLoadingView_tipLabel5",@"正在刷新...");
            }else{
                _tipLabel.text = LocalizedString(@"CustomLoadingView_tipLabel6",@"正在载入更多...");
            }

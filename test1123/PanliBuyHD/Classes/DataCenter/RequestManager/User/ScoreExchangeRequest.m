...
@implementation ScoreExchangeRequest
- (id)init
{
    self = [super init];
    if (self)
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_USER_SCOREEXCHANGE];
...
- (ErrorInfo *)checkResponseError:(NSDictionary*)jsonValue
{
    ...
    if (errorInfo.code == SERVER_SUCCESS)
    {
        ...
        {
            errorInfo.code = 2;
            errorInfo.message = LocalizedString(@"ScoreExchangeRequest_ErrorInfo_code2",@"积分不足");
        }
        else if ([code intValue] == 3)
        {
            errorInfo.code = 3;
            errorInfo.message = LocalizedString(@"ScoreExchangeRequest_ErrorInfo_code3",@"服务费已全免,现在无需兑换优惠券");
        }

...
#import "PageData.h"
@implementation AccountRecordsRequest
- (id)init
{
    self = [super init];
    if (self)
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_USER_ACCOUNTRECORDS];
...
- (void)receive:(ASIHTTPRequest *)request
{
    ...
    if (errorInfo.code == API_SUCCESS)
    {
        ...
        NSArray *arr = [PanliHelper getValue:Q_TYPE_ARRAY inJsonDictionary:dic_result propertyName:@"list"];
        
        PageData *mPageData = [[PageData alloc] init];
        
        NSMutableArray *marr_Result = [[NSMutableArray alloc] init];
        for(NSDictionary *dic in arr)
        {
            UserAccount *mUserAccout = [[UserAccount alloc] initWithDictionary:dic];
            [marr_Result addObject:mUserAccout];
            [mUserAccout release];
        }
        mPageData.list = marr_Result;
        [marr_Result release];
        //获取总条数
        int countRow = [[PanliHelper getValue:Q_TYPE_INT_STRING inJsonDictionary:dic_result propertyName:@"rowCount"] intValue];
        mPageData.rowCount = countRow;
        //接口响应成功
        ((DataRepeater*)self.repeater).isResponseSuccess = YES;
        ((DataRepeater*)self.repeater).responseValue = mPageData;
        [mPageData release];
    }
    else
    {
...
- (ErrorInfo *)checkResponseError:(NSDictionary*)jsonValue
{
    ...
    if (errorInfo.code == SERVER_SUCCESS)
    {
        ...
        {
            errorInfo.code = 99;
            errorInfo.message = LocalizedString(@"Common_ErrorInfo_Short99",@"未知错误");
        }
        else
        {

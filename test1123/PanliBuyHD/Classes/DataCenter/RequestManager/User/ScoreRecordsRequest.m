...
#import "UserScore.h"
#import "PageData.h"
@implementation ScoreRecordsRequest
- (id)init
{
    self = [super init];
    if (self)
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_USER_SCORERECORDS];
...
- (void)receive:(ASIHTTPRequest *)request
{
    ...
    if (errorInfo.code == API_SUCCESS)
    {
        ...
        {
            UserScore *mUserScore = [[UserScore alloc] initWithDictionary:dic];
            [marr_Result addObject:mUserScore];
        }

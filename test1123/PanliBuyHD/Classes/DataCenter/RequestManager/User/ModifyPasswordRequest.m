...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_USER_MODIFYPASSWORD];
...
- (ErrorInfo *)checkResponseError:(NSDictionary*)jsonValue
{
    ...
    if (errorInfo.code == SERVER_SUCCESS)
    {
        ...
        {
            errorInfo.code = 2;
            errorInfo.message = LocalizedString(@"ModifyPasswordRequest_ErrorInfo_code2",@"当前密码错误");
        }
        else if ([code intValue] == 3)
        {
            errorInfo.code = 3;
            errorInfo.message = LocalizedString(@"ModifyPasswordRequest_ErrorInfo_code3",@"新密码格式错误");
        }

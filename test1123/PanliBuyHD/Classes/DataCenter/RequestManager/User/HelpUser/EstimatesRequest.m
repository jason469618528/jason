...
#import "SendType.h"
@implementation EstimatesRequest
- (id)init
{
    self = [super init];
    if (self)
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_ESTIMATES];
...
- (void)receive:(ASIHTTPRequest *)request
{
    ...
    if (errorInfo.code == API_SUCCESS)
    {
        ...
        {
            SendType *mSendType = [[SendType alloc]init];
            mSendType.deliveryId = [[PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:pro propertyName:@"DeliveryID"]intValue];
            mSendType.time = [PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:pro propertyName:@"Time"];
            mSendType.deliveryName = [PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:pro propertyName:@"DeliveryName"];
            mSendType.entryPrice = [[PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:pro propertyName:@"EntryPrice"]floatValue];
            mSendType.productPrice = [[PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:pro propertyName:@"ProductPrice"]floatValue];
            mSendType.shipPrice = [[PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:pro propertyName:@"ShipPrice"]floatValue];
            mSendType.sumPrice = [[PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:pro propertyName:@"SumPrice"]floatValue];
            [marr addObject:mSendType];
            [mSendType release];
        }
        ...
        //设置响应数据
        ((DataRepeater*)self.repeater).responseValue = marr;
        [marr release];
    }
    else
    {
...
- (ErrorInfo *)checkResponseError:(NSDictionary*)jsonValue
{
    ...
    if (errorInfo.code == SERVER_SUCCESS)
    {
        ...
        {
            errorInfo.code = 2;
            errorInfo.message = LocalizedString(@"EstimatesRequest_ErrorInfo_code2",@"内容无效");
        }

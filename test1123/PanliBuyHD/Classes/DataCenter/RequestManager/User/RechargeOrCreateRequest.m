...
@implementation RechargeOrCreateRequest
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_RECHARGEORDERCREATE];
...
- (void)receive:(ASIHTTPRequest *)request
{
    ...
    if (errorInfo.code == API_SUCCESS)
    {
        ...
        NSString* str_OrderNum= [PanliHelper getValue:Q_TYPE_INT_STRING inJsonDictionary:jsonValue propertyName:RP_PARAM_RESULT];
        
        ((DataRepeater*)self.repeater).isResponseSuccess = YES;
        //设置响应数据
        ((DataRepeater*)self.repeater).responseValue = str_OrderNum;
    }
    else
    {
...
- (ErrorInfo *)checkResponseError:(NSDictionary*)jsonValue
{
    ...
    if (errorInfo.code == SERVER_SUCCESS)
    {
        ...
        {
            errorInfo.code = 8;
            errorInfo.message = LocalizedString(@"RechargeOrCreateRequest_ErrorInfo_code8",@"充值方式错误");
        }
        else if ([code intValue] == 9)
        {
            errorInfo.code = 9;
            errorInfo.message = LocalizedString(@"RechargeOrCreateRequest_ErrorInfo_code9",@"订单生成失败");
        }
        else
        {

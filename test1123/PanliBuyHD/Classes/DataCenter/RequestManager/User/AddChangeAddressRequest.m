...
@implementation AddChangeAddressRequest
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:@"User/AddOrUpdateDeliveryAddress.json"];
...
- (void)receive:(ASIHTTPRequest *)request
{
    ...
    {
        NSString *str_Id = [PanliHelper getValue:Q_TYPE_NUMBER inJsonDictionary:jsonValue propertyName:RP_PARAM_RESULT];
        
        ((DataRepeater*)self.repeater).isResponseSuccess = YES;
        //设置响应数据
        ((DataRepeater*)self.repeater).responseValue = str_Id;
    }
    else
    {
...
- (ErrorInfo *)checkResponseError:(NSDictionary*)jsonValue
{
    ...
    if (errorInfo.code == SERVER_SUCCESS)
    {
        ...
        if ([code intValue] == API_SUCCESS)
        {
            ...
            errorInfo.message = LocalizedString(@"AddChangeAddressRequest_ErrorInfo_API_SUCCESS",@"保存成功");
        }
        else if ([code intValue] == 6)
        {
            errorInfo.code = 6;
            errorInfo.message = LocalizedString(@"AddChangeAddressRequest_ErrorInfo_code6",@"保存运送方式失败");
        }
        else if ([code intValue] == 7)
        {
            errorInfo.code = 7;
            errorInfo.message = LocalizedString(@"AddChangeAddressRequest_ErrorInfo_code7",@"地址过长，保存失败");
        }

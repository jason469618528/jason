...
@implementation UpdataUserConfigRequest
- (id)init
{
    self = [super init];
    if (self)
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_UPDATEUSERCONFIG];
...
- (void)receive:(ASIHTTPRequest *)request
{
    ...
    {
        //接口响应成功
        NSString *arr = [PanliHelper getValue:Q_TYPE_INT_STRING inJsonDictionary:jsonValue propertyName:RP_PARAM_RESULT];
        
        
        ((DataRepeater*)self.repeater).isResponseSuccess = YES;
        //设置响应数据
        ((DataRepeater*)self.repeater).responseValue = arr;
    }
    else
    {

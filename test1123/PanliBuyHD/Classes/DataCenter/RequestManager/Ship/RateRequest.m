...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_SHIPRATE];
...
- (ErrorInfo *)checkResponseError:(NSDictionary*)jsonValue
{
    ...
    if (errorInfo.code == SERVER_SUCCESS)
    {
        ...
        {
            errorInfo.code = 3;
            errorInfo.message = LocalizedString(@"RateRequest_ErrorInfo_code3",@"运单编号无效");
        }
        else if ([code intValue] == 10)
        {
            errorInfo.code = 10;
            errorInfo.message = LocalizedString(@"RateRequest_ErrorInfo_code10",@"运单已评价");
        }
        else if ([code intValue] == 11)
        {
            errorInfo.code = 11;
            errorInfo.message = LocalizedString(@"RateRequest_ErrorInfo_code11",@"运单不允许评价");
        }
        else if ([code intValue] == 12)
        {
            errorInfo.code = 12;
            errorInfo.message = LocalizedString(@"RateRequest_ErrorInfo_code12",@"运单评价失败");
        }

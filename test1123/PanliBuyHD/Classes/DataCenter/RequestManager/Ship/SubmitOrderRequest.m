...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_SUBMITSHIPORDER];
...
- (void)receive:(ASIHTTPRequest *)request
{
    ...
    if (errorInfo.code == API_SUCCESS)
    {
        ...
        [PanliHelper updataUserBalance:[[dic objectForKey:@"Balance"] floatValue]];
...
- (ErrorInfo *)checkResponseError:(NSDictionary*)jsonValue
{
    ...
    if (errorInfo.code == SERVER_SUCCESS)
    {
        ...
        else if ([code intValue] == 9)
        {
            errorInfo.code = 9;
            errorInfo.message = LocalizedString(@"SubmitOrderRequest_ErrorInfo_code9",@"运送方案无效");
        }
        else if ([code intValue] == 12)
        {
            errorInfo.code = 12;
            errorInfo.message = LocalizedString(@"SubmitOrderRequest_ErrorInfo_code12",@"支付失败");
        }
        else if ([code intValue] == 14)
        {
            errorInfo.code = 14;
            errorInfo.message = LocalizedString(@"SubmitOrderRequest_ErrorInfo_code14",@"余额不足");
        }
        else if ([code intValue] == 15)
        {
            errorInfo.code = 15;
            errorInfo.message = LocalizedString(@"SubmitOrderRequest_ErrorInfo_code15",@"无效优惠券");
        }
        else if ([code intValue] == 16)
        {
            errorInfo.code = 16;
            errorInfo.message = LocalizedString(@"SubmitOrderRequest_ErrorInfo_code16",@"免费赠品无效");
        }
        else if ([code intValue] == 17)
        {
            errorInfo.code = 17;
            errorInfo.message = LocalizedString(@"SubmitOrderRequest_ErrorInfo_code17",@"缺少用户积分");
        }
        else if ([code intValue] == 98)
        {
            errorInfo.code = 98;
            errorInfo.message = LocalizedString(@"SubmitOrderRequest_ErrorInfo_code98",@"无效输入");
        }

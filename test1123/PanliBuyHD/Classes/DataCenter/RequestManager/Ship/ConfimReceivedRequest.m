...
@implementation ConfimReceivedRequest
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_CONFIRMRECEIVED];
...
- (void)receive:(ASIHTTPRequest *)request
{
    ...
    if (errorInfo.code == API_SUCCESS)
    {
        ...
        NSString * acquireScore=[[dic objectForKey:@"acquireScore"]retain];
        
        
        ((DataRepeater*)self.repeater).isResponseSuccess = YES;
        //设置响应数据
        ((DataRepeater*)self.repeater).responseValue = acquireScore;
        [acquireScore release];
    }
    else
    {
...
- (ErrorInfo *)checkResponseError:(NSDictionary*)jsonValue
{
    ...
    if (errorInfo.code == SERVER_SUCCESS)
    {
        ...
        {
            errorInfo.code = 3;
            errorInfo.message = LocalizedString(@"ConfimReceivedRequest_ErrorInfo_code3",@"运单编号无效");
        }
        else if ([code intValue] == 12)
        {
            errorInfo.code = 12;
            errorInfo.message = LocalizedString(@"ConfimReceivedRequest_ErrorInfo_code12",@"确认收货失败");
        }
        else if ([code intValue] == 13)
        {
            errorInfo.code = 13;
            errorInfo.message = LocalizedString(@"ConfimReceivedRequest_ErrorInfo_code13",@"该运单不允许确认收货");
        }

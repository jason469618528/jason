...
@implementation SetSysMsgIsReadRequest
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_SETSYSMESSAGEISREAD];
...
- (ErrorInfo *)checkResponseError:(NSDictionary*)jsonValue
{
    ...
    if (errorInfo.code == SERVER_SUCCESS)
    {
        ...
        {
            errorInfo.code = 2;
            errorInfo.message = LocalizedString(@"SetSysMsgIsReadRequest_ErrorInfo_code2",@"TopicId无效");
        }
        else
        {

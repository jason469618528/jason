...
@implementation MessageTopicCsRequest
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_USERMESSAGETOPICS];
...
- (void)receive:(ASIHTTPRequest *)request
{
    ...
    if (errorInfo.code == API_SUCCESS)
    {
        ...
        for (NSDictionary *dic in arr)
        {
            ...
            MessageTopic * messageTopic = [[MessageTopic alloc] init];
            messageTopic.topicId = [[PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"TopicId"] intValue];
            messageTopic.title = [PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"Title"];
            messageTopic.objId = [PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"ObjId"];
            messageTopic.lastMessageTime = [PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"LastMessageTime"];
            messageTopic.objType = [[PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"ObjType"]intValue];
            messageTopic.isRead = [[PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"IsRead"] boolValue];
            messageTopic.lastMessageContent = [PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"LastMessageContent"];
            messageTopic.image = [PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"ObjImage"];
            messageTopic.lastSender = [PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"LastSender"];
            messageTopic.lastMessageTime_zhCn = [PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"LastMessageTime_zhCN"];
            [messageArr addObject:messageTopic];
            [messageTopic release];
        }
        
        ((DataRepeater*)self.repeater).isResponseSuccess = YES;
        //设置响应数据
        ((DataRepeater*)self.repeater).responseValue = messageArr;
        [messageArr release];
    }
    else
    {

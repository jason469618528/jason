...
@implementation SendMsgByObjectIdRequest
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_SENDMESSAGEBYOBJECTID];
...
- (ErrorInfo *)checkResponseError:(NSDictionary*)jsonValue
{
    ...
    if (errorInfo.code == SERVER_SUCCESS)
    {
        ...
        {
            errorInfo.code = 4;
            errorInfo.message = LocalizedString(@"SendMsgByObjectIdRequest_ErrorInfo_code4",@"对象ID无效");
        } else if ([code intValue] == 3)
        {
            errorInfo.code = 3;
            errorInfo.message = LocalizedString(@"SendMsgByObjectIdRequest_ErrorInfo_code3",@"短信内容为空");
        }  else if ([code intValue] == 5)
        {
            errorInfo.code = 5;
            errorInfo.message = LocalizedString(@"SendMsgByObjectIdRequest_ErrorInfo_code5",@"发送失败");
        }
        else
        {

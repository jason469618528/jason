...
@implementation MessageObjectcsRequest
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_CONSULTINGMESSAGELISTBYOBJECTID];
...
- (void)receive:(ASIHTTPRequest *)request
{
    ...
    if (errorInfo.code == API_SUCCESS)
    {
        ...
        NSMutableArray * messageDetailArr = [[NSMutableArray alloc] init];
        for (NSDictionary * dic in arr)
        {
            
            Message * messageDetail = [[Message alloc] init];
            
            messageDetail.messageId = [[PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"ID"] intValue];
            messageDetail.content = [PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"Content"];
            messageDetail.dateCreated = [PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"DateCreated"];
            messageDetail.kefuName = [PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"KefuName"];
            messageDetail.isOwn = [[PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"IsOwn"]boolValue];
            [messageDetailArr addObject:messageDetail];
            [messageDetail release];
        }
        ((DataRepeater*)self.repeater).isResponseSuccess = YES;
        //设置响应数据
        ((DataRepeater*)self.repeater).responseValue = messageDetailArr;
        [messageDetailArr release];
    }
    else
    {

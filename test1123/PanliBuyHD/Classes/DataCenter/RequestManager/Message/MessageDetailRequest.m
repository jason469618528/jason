...
@implementation MessageDetailRequest
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_USERMESSAGEDETAIL];
...
- (void)receive:(ASIHTTPRequest *)request
{
    ...
    if (errorInfo.code == API_SUCCESS)
    {
        ...
        for (NSDictionary *dic in arr)
        {
            ...
            [detailArr addObject:messageDetail];
            [messageDetail release];
        }
        
        ((DataRepeater*)self.repeater).isResponseSuccess = YES;
        //设置响应数据
        ((DataRepeater*)self.repeater).responseValue = detailArr;
        [detailArr release];
    }
    else
    {
...
- (ErrorInfo *)checkResponseError:(NSDictionary*)jsonValue
{
    ...
    if (errorInfo.code == SERVER_SUCCESS)
    {
        ...
        {
            errorInfo.code = 2;
            errorInfo.message = LocalizedString(@"MessageDetailRequest_ErrorInfo_code2",@"短信主题ID无效");
        }
        else
        {

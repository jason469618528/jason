...
@implementation ShareBuyPraiseRequest
- (id)init
{
    self = [super init];
    if (self)
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_SHARE_SHAREBUYPRAISE];
...
- (void)receive:(ASIHTTPRequest *)request
{
    ...
    if (errorInfo.code == API_SUCCESS)
    {
        ...
        //接口响应成功
        //        NSDictionary *result = [PanliHelper getValue:Q_TYPE_DICTIONARY inJsonDictionary:jsonValue propertyName:RP_PARAM_RESULT];
        //接口响应成功
...
- (ErrorInfo *)checkResponseError:(NSDictionary*)jsonValue
{
    ...
    if (errorInfo.code == SERVER_SUCCESS)
    {
        ...
        {
            errorInfo.code = 4;
            errorInfo.message = LocalizedString(@"ShareBuyPraiseRequest_ErrorInfo_code4",@"亲,赞过了");
        }
        else if ([code intValue] == 5)
        {
            errorInfo.code = 5;
            errorInfo.message = LocalizedString(@"ShareBuyPraiseRequest_ErrorInfo_code5",@"商品ID出错");
        }

//
//  ShareBuyIsPraisedBySelfRequest.m
//  PanliApp
//
//  Created by jason on 13-12-25.
//  Copyright (c) 2013年 Panli. All rights reserved.
//

#import "ShareBuyIsPraisedBySelfRequest.h"

@implementation ShareBuyIsPraisedBySelfRequest
- (id)init
{
    self = [super init];
    if (self)
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_SHARE_ISPRAISEDBYSELF];
...
- (void)receive:(ASIHTTPRequest *)request
{
    ...
    {
        //接口响应成功
        NSString *str_result = [PanliHelper getValue:Q_TYPE_INT_STRING inJsonDictionary:jsonValue propertyName:RP_PARAM_RESULT];
        
        ((DataRepeater*)self.repeater).isResponseSuccess = YES;
        //设置响应数据
        ((DataRepeater*)self.repeater).responseValue = str_result;
    }
    else
    {

//
//  ShareBuyProductDetailRequest.m
//  PanliApp
//
...
@implementation ShareBuyProductDetailRequest
- (id)init
{
    self = [super init];
    if (self)
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_SHARE_USERSHAREDETAILS];
...
- (void)receive:(ASIHTTPRequest *)request
{
    ...
    if (errorInfo.code == API_SUCCESS)
    {
        ...
        NSMutableArray *mArr_DetailList = [[NSMutableArray alloc] init];
        for(NSDictionary *dic in arr_result)
        {
            ShareBuyDetail *mShareBuyDetail = [[ShareBuyDetail alloc] initWithDictionary:dic];
            [mArr_DetailList addObject:mShareBuyDetail];
            [mShareBuyDetail release];
        }
        ((DataRepeater*)self.repeater).isResponseSuccess = YES;
        //设置响应数据
        ((DataRepeater*)self.repeater).responseValue = mArr_DetailList;
        [mArr_DetailList release];
    }
    else
    {

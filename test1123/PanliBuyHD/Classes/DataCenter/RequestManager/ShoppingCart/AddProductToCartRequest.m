...
@implementation AddProductToCartRequest
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_ADDTOCART];
...
- (ErrorInfo *)checkResponseError:(NSDictionary*)jsonValue
{
    ...
    if (errorInfo.code == SERVER_SUCCESS)
    {
        ...
        if ([code intValue] == API_SUCCESS)
        {
            ...
            errorInfo.message = LocalizedString(@"AddProductToCartRequest_ErrorInfo_API_SUCCESS",@"添加成功");
        }
        else if ([code intValue] == 2)
        {
            errorInfo.code = 2;
            errorInfo.message = LocalizedString(@"AddProductToCartRequest_ErrorInfo_code2",@"添加失败");
        }
        else if ([code intValue] ==11)
        {
            errorInfo.code = 11;
            errorInfo.message = LocalizedString(@"AddProductToCartRequest_ErrorInfo_code11",@"请求超时");
        }

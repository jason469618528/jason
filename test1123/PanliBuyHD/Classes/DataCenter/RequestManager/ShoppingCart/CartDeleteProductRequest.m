...
@implementation CartDeleteProductRequest
- (id)init
{
    self = [super init];
    if (self)
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_SHOPCART_DELETEPRODUCTS];
...
- (void)receive:(ASIHTTPRequest *)request
{
    ...
    if (errorInfo.code == API_SUCCESS)
    {
        ...
        else
        {
            ...
            errorInfo.message = LocalizedString(@"CartDeleteProductRequest_ErrorInfo_code10000",@"删除商品失败");
        }
    }
    else
    {
...
- (ErrorInfo *)checkResponseError:(NSDictionary*)jsonValue
{
    ...
    if (errorInfo.code == SERVER_SUCCESS)
    {
        ...
        {
            errorInfo.code = 3;
            errorInfo.message = LocalizedString(@"CartDeleteProductRequest_ErrorInfo_code3",@"商品ID无效");
        }
        else if ([code intValue] == 4)
        {
            errorInfo.code = 4;
            errorInfo.message = LocalizedString(@"CartDeleteProductRequest_ErrorInfo_code4",@"该商品不能撤销");
        }

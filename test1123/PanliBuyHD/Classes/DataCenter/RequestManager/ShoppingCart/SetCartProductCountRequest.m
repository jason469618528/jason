//
//  SetCartProductCountRequest.m
//  PanliApp
//
//  Created by jason on 13-5-8.
//  Copyright (c) 2013年 Panli. All rights reserved.
//

#import "SetCartProductCountRequest.h"

@implementation SetCartProductCountRequest
- (id)init
{
    self = [super init];
    if (self)
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_SHOPCART_PRODUCTCOUNT];
...
- (void)receive:(ASIHTTPRequest *)request
{
    ...
    {
        //接口响应成功
        BOOL result = [[PanliHelper getValue:Q_TYPE_BOOLEAN inJsonDictionary:jsonValue propertyName:RP_PARAM_RESULT] boolValue];
        ...
        else
        {
            ...
            ((DataRepeater*)self.repeater).responseValue = nil;
            errorInfo.code = 10000;
            errorInfo.message = LocalizedString(@"SetCartProductCountRequest_ErrorInfo_code10000",@"修改数量失败");
        }
    }
    else
    {
...
- (ErrorInfo *)checkResponseError:(NSDictionary*)jsonValue
{
    ...
    if (errorInfo.code == SERVER_SUCCESS)
    {
        ...
        {
            errorInfo.code = 3;
            errorInfo.message = LocalizedString(@"SetCartProductCountRequest_ErrorInfo_code3",@"商品ID无效");
        }
        else if ([code intValue] == 4)
        {
            errorInfo.code = 4;
            errorInfo.message = LocalizedString(@"SetCartProductCountRequest_ErrorInfo_code4",@"该商品不能修改");
        }

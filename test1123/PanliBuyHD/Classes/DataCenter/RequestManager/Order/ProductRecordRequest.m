...
#import "ProductRecordRequest.h"
#import "ProductStatusRecord.h"

@implementation ProductRecordRequest
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_GETPRODUCTSTATUSRECORD];
...
- (void)receive:(ASIHTTPRequest *)request
{
    ...
    if (errorInfo.code == API_SUCCESS)
    {
        ...
        {
            ProductStatusRecord * productRecord = [[ProductStatusRecord alloc] init];
            productRecord.flowStatusRecordId = [[PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"Id"] intValue];
            productRecord.remark = [PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"Remark"];
            productRecord.dataCreated = [PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"DataCreated"];
            productRecord.productId = [[PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"ProductId"] intValue];
            productRecord.status = [[PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"Status"] intValue];
            [recordArr addObject:productRecord];
        }
...
- (ErrorInfo *)checkResponseError:(NSDictionary*)jsonValue
{
    ...
    if (errorInfo.code == SERVER_SUCCESS)
    {
        ...
        {
            errorInfo.code = 2;
            errorInfo.message = LocalizedString(@"ProductRecordRequest_ErrorInfo_code2",@"商品ID无效");
        }
        else
        {

...
#import "ProductExpressRequest.h"
#import "ExpressInfo.h"


@implementation ProductExpressRequest
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_GETPRODUCTEXPRESS];
...
- (ErrorInfo *)checkResponseError:(NSDictionary*)jsonValue
{
    ...
    if (errorInfo.code == SERVER_SUCCESS)
    {
        ...
        {
            errorInfo.code = 3;
            errorInfo.message = LocalizedString(@"ProductExpressRequest_ErrorInfo_code3",@"物流地址无效");
            
        } else if ([code intValue] == 4)
        {
            errorInfo.code = 4;
            errorInfo.message = LocalizedString(@"ProductExpressRequest_ErrorInfo_code4",@"物流编号无效");

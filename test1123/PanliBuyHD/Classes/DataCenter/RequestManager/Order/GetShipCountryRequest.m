...
@implementation GetShipCountryRequest
...
{
    //拼接URL
    NSString *url = [BASE_URL stringByAppendingString:RQNAME_GETCOUNTRY];
...
- (void)receive:(ASIHTTPRequest *)request
{
    ...
    {
        DLOG(@"=====GetShipCountryList success=====");
        //接口响应成功
        ...
        {
            ShipCountry *m_Country = [[ShipCountry alloc]init];
            m_Country.shipCountryId = [[PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"ShipCountryId"] intValue];
            m_Country.initial = [PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"Initial"];
            m_Country.name = [PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"Name"];
            m_Country.code = [PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"Code"];
            m_Country.order = [[PanliHelper getValue:Q_TYPE_STRING inJsonDictionary:dic propertyName:@"ID"] intValue];
            m_Country.isCommon = [[PanliHelper getValue:Q_TYPE_BOOLEAN inJsonDictionary:dic propertyName:@"IsCommon"] boolValue];
            [recordArr addObject:m_Country];
            
        }
        [GlobalObj setShipCountry:recordArr];
...
- (ErrorInfo *)checkResponseError:(NSDictionary*)jsonValue
{
    ...
    if (errorInfo.code == SERVER_SUCCESS)
    {
        ...
        {
            errorInfo.code = 2;
            errorInfo.message = LocalizedString(@"GetShipCountryRequest_ErrorInfo_code2",@"商品ID无效");
        }
        else
        {
